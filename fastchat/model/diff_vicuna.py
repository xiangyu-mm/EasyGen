...
class DiffLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        self.clip_proj = Mlp_Clip(in_features=4096, hidden_features=4096 * 4, out_features=4096, act_layer=nn.GELU,
                                  drop=0.)
    ...
    def proj_image(self, tmp):
        ...
        # inputs_images = self.clip_proj(clip)
        # inputs_fastchat = torch.cat((inputs_images, inputs_captions))
    ...
    def forward(
            ...
            clip: Optional[torch.FloatTensor] = None,
            clip_l: Optional[torch.FloatTensor] = None,
        ...
        if images is not None:
            ...
            caption_embeds = self.fastchat_proj(images)
            inputs_images = self.clip_proj(clip_l)
            img_embeds = torch.cat((inputs_images, caption_embeds), dim=1)
        ...
        if have_image is None:
            prompt = 'USER: '
            prompt_tokens = self.tokenizer(
                prompt, return_tensors="pt", max_length=None).to(input_ids.device)

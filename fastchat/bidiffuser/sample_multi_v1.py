...
import torch
import random
import fastchat.bidiffuser.utils as utils
from fastchat.bidiffuser.dpm_solver_pp import NoiseScheduleVP, DPM_Solver
from absl import logging
import einops
import fastchat.bidiffuser.libs.autoencoder as autoencoder
import fastchat.bidiffuser.libs.clip
...
def evaluate(config):
    ...
    def t2i_nnet(x, timesteps, text):  # text is the low dimension version of the text clip embedding
        ...
        if config.sample.t2i_cfg_mode == 'empty_token':
            ...
            z_out_uncond, clip_img_out_uncond, text_out_uncond = nnet(z, clip_img, text=_empty_context, t_img=timesteps,
                                                                      t_text=t_text,
                                                                      data_type=torch.zeros_like(t_text, device=device,
                                                                                                 dtype=torch.int) + config.data_type)
    ...
    def t_nnet(x, timesteps):
        ...
        z_out, clip_img_out, text_out = nnet(z, clip_img, text=x, t_img=torch.ones_like(timesteps) * N,
                                             t_text=timesteps,
                                             data_type=torch.zeros_like(timesteps, device=device,
                                                                        dtype=torch.int) + config.data_type)
        return text_out
    ...
    def i2t_nnet(x, timesteps, z, clip_img):
        ...
        z_out, clip_img_out, text_out = nnet(z, clip_img, text=x, t_img=t_img, t_text=timesteps,
                                             data_type=torch.zeros_like(t_img, device=device,
                                                                        dtype=torch.int) + config.data_type)
        ...
        z_out_uncond, clip_img_out_uncond, text_out_uncond = nnet(z_N, clip_img_N, text=x,
    ...
    def joint_nnet(x, timesteps):
        ...
        _, _, text_out_uncond = nnet(z_noise, clip_img_noise, text=text, t_img=torch.ones_like(timesteps) * N,
                                     t_text=timesteps,
                                     data_type=torch.zeros_like(timesteps, device=device,
                                                                dtype=torch.int) + config.data_type)
        z_out_uncond, clip_img_out_uncond, _ = nnet(z, clip_img, text=text_noise, t_img=timesteps,
                                                    t_text=torch.ones_like(timesteps) * N,
                                                    data_type=torch.zeros_like(timesteps, device=device,
                                                                               dtype=torch.int) + config.data_type)
    ...
    contexts_low_dim = contexts if not use_caption_decoder else caption_decoder.encode_prefix(
        contexts)  # the low dimensional version of the contexts, which is the input to the nnet
...
def get_feature(
    ...
    clip_img_model, clip_img_model_preprocess = clip_img_model, clip_img_model_preprocess

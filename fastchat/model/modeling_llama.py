...
class LlamaForCausalLM(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.caption_decoder = CaptionDecoder(device="cuda",
                                              pretrained_path="/home/data2/xiangyu/Code/unidiffuser/models"
                                                              "/caption_decoder.pth",
                                              hidden_dim=64)
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt):
        if prompt:
            ...
            p_after_embeds = self.model.embed_tokens(p_after_tokens.input_ids). \
                expand(batch_size, -1, -1)
    ...
    def forward(
        ...
        inputs_embeds: Optional[torch.FloatTensor] = None,
        query_embeds: Optional[torch.FloatTensor] = None,
        masked_labels: Optional[torch.FloatTensor] = None,
        ...
        device = original_labels[-1].device
        img_embeds = self.fastchat_proj(original_labels)
        ...
        # )
        targets = torch.cat([empty_targets, masked_labels], dim=1)
        batch_size = img_embeds.shape[0]
        ...
        atts_bos = atts_img[:, :1]
        to_regress_embeds = self.model.embed_tokens(to_regress_tokens.input_ids)

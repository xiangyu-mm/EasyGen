...
class MSCOCODatabase_kap(Dataset):
    ...
    def _load_image(self, key: int):
        id = self.coco.loadImgs(key)[0]["id"]
        path = "COCO_val2014_" + str(id).rjust(12, '0') + ".jpg"
...
class VisDialDatabase(Dataset):
    def __init__(self, root, annFile, size=None):
        self.root = root
        self.height = self.width = size
        self.annotation = json.load(open(annFile))
        self.dialogs = self.annotation['data']['dialogs']
        self.answers = self.annotation['data']['answers']
        self.questions = self.annotation['data']['questions']

    def __len__(self):
        return len(self.annotation)

    def _load_image(self, key: int):
        # path = "VisualDialog_val2018_" + str(key).rjust(12, '0') + ".jpg"
        try:
            self.root = '/home/data2/xiangyu/Data/visdial/VisualDialog_val2018'
            path = "VisualDialog_val2018_" + str(key).rjust(12, '0') + ".jpg"
            return Image.open(os.path.join(self.root, path)).convert("RGB")
        except:
            self.root = '/home/data2/xiangyu/Data/val2014'
            path = "COCO_val2014_" + str(key).rjust(12, '0') + ".jpg"
            return Image.open(os.path.join(self.root, path)).convert("RGB")

    def __getitem__(self, index):
        key = self.dialogs[index]['image_id']
        dialog = self.dialogs[index]['dialog']
        caption = self.dialogs[index]['caption']
        conv = []
        for i in range(len(dialog)):
            dialog[i]['answer'] = self.answers[dialog[i]['answer']]
            dialog[i]['question'] = self.questions[dialog[i]['question']]
            conv.append({'question': dialog[i]['question'], 'answer': dialog[i]['answer']})
        image = self._load_image(key)

        return image, conv, caption, key


class LLaVADatabase(Dataset):
    def __init__(self, root, annFile, size=None):
        self.root = root
        self.height = self.width = size
        self.annotation = json.load(open(annFile))
    ...
    def _load_image(self, key):
        # path = "VisualDialog_val2018_" + str(key).rjust(12, '0') + ".jpg"
        # self.root = '/home/data2/xiangyu/InstructTuning/Data/train2017'
        path = key
        return Image.open(os.path.join(self.root, path)).convert("RGB")

    def __getitem__(self, index):
        conv_id = self.annotation[index]['id']
        image = self.annotation[index]['image']
        dialog = self.annotation[index]['conversations']
        image = self._load_image(image)

        return image, dialog, conv_id


class OKVQADatabase(Dataset):
    def __init__(self, root, annFile, size=None):
        self.root = root
        self.height = self.width = size
        self.annotation = json.load(open(annFile))
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.root, ann["image"])
...
def get_feature_dir_info(root):
    files_caption = glob.glob(os.path.join(root, '*_0.npy'))
    num_data = len(files_caption)
    return num_data
...
class MSCOCOFeatureDataset(Dataset):
    ...
    def __init__(self, root):
        self.root = root
        self.num_data = get_feature_dir_info(root)
    ...
    def __getitem__(self, index):
        ...
        clip = np.load(os.path.join(self.root, f'{index}_img.npy'))
        k = 0
...
class MSCOCO256Features(
    ...
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset(os.path.join(path, 'val_unidiffuser'))
        self.test = MSCOCOFeatureDataset(os.path.join(path, 'val_unidiffuser'))
        # assert len(self.train) == 82783
        assert len(self.train) == 40504
    ...
    @property
    def fid_stat(self):
        return f'/home/data2/xiangyu/Data/coco512_features/fid_stats/fid_stats_mscoco256_val.npz'

...
class DatasetVQAV2WithClip(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetVQAV2WithClip, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/vqav2"
        self.num_data = 10000
    ...
    def __getitem__(self, index):
        ...
        target = input_ids.clone()
        target[:instruction_len - 2] = IGNORE_TOKEN_ID
        data_dict = dict(input_ids=input_ids, labels=target, images=diff, clip_l=clip_l)
        return data_dict


class DatasetTextVQAWithClip(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetTextVQAWithClip, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/textvqa"
        self.num_data = get_feature_test(self.root)
    ...
    def __getitem__(self, index):
        ...
        question = str(question).strip().replace('\n', '') + ' please giving an short answer.'
        # answer = question + ' \n' + '### ASSISTANT: ' + str(answer[0]) + '\n'
        answer = question + ' ASSISTANT: ' + str(answer[1]) + '</s>'
        instruction_len = len(self.tokenizer(question + ' ASSISTANT: ').input_ids)
        input_ids = self.tokenizer(
            answer,
            return_tensors="pt",
        ...
        target = input_ids.clone()
        target[:instruction_len - 2] = IGNORE_TOKEN_ID
        # question = input_ids[:instruction_len - 2]
        data_dict = dict(input_ids=input_ids, labels=target, images=diff,
                         clip_l=clip_l, question=input_ids[:instruction_len - 2])
        return data_dict


class DatasetTextCaptionWithClip(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetTextCaptionWithClip, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/textvqa_train"
        self.num_data = 7587
    ...
    def __getitem__(self, index):
        # c = np.load(os.path.join(self.root, f'{head}_{tail}.npy'))
        # v = np.load(os.path.join(self.root, f'{head}_{tail}_llm.npy'))
        text = np.load(os.path.join(self.root, f'{index}_caption.npy'))
...
class DatasetTextCaptionWithClip2(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetTextCaptionWithClip2, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/textvqa"
        self.num_data = get_feature_test(self.root)
...
class DatasetTextVQAWithClip2(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetTextVQAWithClip2, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/textvqa_train"
        self.num_data = get_feature_test(self.root)
...
class DatasetLLaVAWithClip(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        super(DatasetLLaVAWithClip, self).__init__()
        self.tokenizer = tokenizer
        self.root = "/home/data2/xiangyu/InstructTuning/Data/LLaVA_80K"
        self.num_data = 2000
...
def make_supervised_data_module_clip(tokenizer: transformers.PreTrainedTokenizer,
                                     data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    dataset_textvqa = DatasetTextVQAWithClip
    textvqa_dataset = dataset_textvqa(tokenizer=tokenizer)
    dataset_textvqa2 = DatasetTextVQAWithClip2
    textvqa_dataset2 = dataset_textvqa2(tokenizer=tokenizer)
    dataset_vqav2 = DatasetVQAV2WithClip
    vqav2_dataset = dataset_vqav2(tokenizer=tokenizer)
    dataset_llava = DatasetLLaVAWithClip
    llava_dataset = dataset_llava(tokenizer=tokenizer)
    textcap_dataset = DatasetTextCaptionWithClip
    dataset_textcap = textcap_dataset(tokenizer=tokenizer)
    textcap_dataset2 = DatasetTextCaptionWithClip2
    dataset_textcap2 = textcap_dataset2(tokenizer=tokenizer)
    train_dataset = textvqa_dataset2 + llava_dataset + dataset_textcap \
                    + dataset_textcap2 + dataset_textcap + textvqa_dataset + vqav2_dataset
    # data_collator = DataCollatorForSupervisedDataset2014(tokenizer=tokenizer)
    data_collator = DataCollatorForClipDataset(tokenizer=tokenizer)
    return dict(train_dataset=train_dataset,
                eval_dataset=None,
                data_collator=data_collator)
...
@dataclass
class DataCollatorForClipDataset(object):
    """Collate examples for supervised fine-tuning."""
...
from fastchat.model.diff_clip import DiffLlamaForCausalLM
...
def train():
    ...
    if training_args.lora:
        ...
        pretext_model = torch.load('/home/data2/xiangyu/Code/EasyGen/easy_clip/checkpoint-1800/pytorch_model.bin')

        pre_model = {key[17:]: value for key, value in pretext_model.items() if
                     ('clip_proj' in key or 'fastchat_proj' in key)}
        model_dict.update(pre_model)
        model.load_state_dict(model_dict)

        model = PeftModel.from_pretrained(
            model,
            "/home/data2/xiangyu/Code/EasyGen/easy_clip",
            is_trainable=True
        )
    ...
    # Load data
    data_module = make_supervised_data_module_clip(tokenizer=tokenizer, data_args=data_args)
